...
/*
    ...
    - Uporządkować kod związany z opcją Magazyn
    - Zastąpić if i elsy związane z zakupem danego sprzętu na metodę Zakup klasy Produkt
    - Stworzyć metode faktury dla klienta aby zastąpić goto fakturka
...
/*		Wybor do podjecia		*/
	...
	if (!(std::cin >> op))
    { /* handle error */ string wyjatek_op = "Podaj poprawna liczbe! "; throw 1; }
...
/*		Opcje do wykonania		*/
	...
	switch(wybor_operacji)
    {
		...
		case Sprzedaz: /* Sprzedaz */
		{
            ...
            int wyb = 0;
            if (!(std::cin >> wyb))
            { /* handle error */ string wyjatek = "Podaj poprawna liczbe! "; throw 1; }
		  ...
		  /*	Tu nastepuje zapisanie do pliku zwanego koszyk.txt		*/
					...
					cout << "\nTwoj wybor:";
					int wyb_router=0;
                    if (!(std::cin >> wyb_router))
                    { /* Przechwycony błąd */ string wyjatek = "Podaj poprawna liczbe! "; throw 1; }
		        ...
		        /*	Tu nastepuje zapisanie do pliku zwanego koszyk.txt		*/
...

					{
							...
							int wyb_switch=0;
                            if (!(std::cin >> wyb_switch)) { /* handle error */ string wyjatek = "Podaj poprawna liczbe! "; throw 1; }
...
/*	Zakonczenie switcha z wyborem produktu(laptop,komputer,switch,router)	*/
		...
		case Magazyn: /* Magazyn */
		{
			do
            {
                ...
                switch(wybor_magazyn)
                {
                    case 1:
                    {
                        ...
                        {
                            cout << t.v[a].nazwa << " " << dostepnosc(t.v[a].ilosc) << "\n";
                        }
                        getchar();
                        break;
                    }
            ...
            /*<-- TU stworzyć fakture kupna oraz zapisywanie(aktualizacja) do pliku magazyn.txt -->*/
...
*/				  
           ...
           if(danaNazwa.empty())
           {
               for(unsigned int i = 0; i < t.v.size(); ++i)
               {
                   ...
                   danaNazwa.push_back(t.v[i].nazwa);
                   danaCena.push_back(t.v[i].cena);
                   danaWybor.push_back(t.v[i].koszyk);
               }
           }
           else
           {
